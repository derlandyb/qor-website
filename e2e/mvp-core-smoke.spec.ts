 ...
 * all 4 cities, so "Vitória" (the /eventos default active city) always
...
test("MVP Core: browse -> filter -> event detail -> signup -> verify (OTP) -> login -> profile", async ({ page }) => {
  // --- Step 1: browse the home feed (no city filter here anymore — the
  // home page shows the soonest event across all cities via HeroFeature,
  // followed by the "Próximos eventos" carousel) ---
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();
  await expect(page.locator('a[href^="/eventos/"]').first()).toBeVisible();

  // --- Step 2: browse /eventos, filtered to Vitória (the default city) ---
  ...
  // --- Step 3: open the event detail page ---
  await firstEventLink.click();
  await expect(page).toHaveURL(/\/eventos\/\d+$/);
  await expect(page.getByRole("heading", { level: 1 })).toBeVisible();

  // --- Step 4: sign up for a new fan account ---
  ...
  // --- Step 5: verify the account with the fixed local/testing OTP code ---
